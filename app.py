    ...
    with admin_tab2:
        ...
        if selected_option == "⚠️ [미신청] 수강신청 미완료 학생 목록":
            ...
            if not unenrolled_list:
                ...
                df_unenrolled = pd.DataFrame(rows)
                st.dataframe(df_unenrolled, use_container_width=True)
                
                # Excel Download
                u_output = io.BytesIO()
                with pd.ExcelWriter(u_output, engine='openpyxl') as writer:
                    df_unenrolled.to_excel(writer, index=False, sheet_name='미신청자목록')
                u_excel_data = u_output.getvalue()
                
                st.download_button(
                    label="📥 미신청자 명단 다운로드 (Excel)",
                    data=u_excel_data,
                    file_name="수강신청_미신청자_명단.xlsx",
            ...
            if not enrolled_list:
                ...
                df_enrolled = pd.DataFrame(rows)
                st.dataframe(df_enrolled, use_container_width=True)
                
                # Excel Download
                e_output = io.BytesIO()
                with pd.ExcelWriter(e_output, engine='openpyxl') as writer:
                    df_enrolled.to_excel(writer, index=False, sheet_name='수강생목록')
                e_excel_data = e_output.getvalue()
                
                st.download_button(
                    label=f"📥 {selected_course['name']} 수강생 명단 다운로드 (Excel)",
                    data=e_excel_data,
                    file_name=f"{selected_course['name']}_수강생_명단.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key=f"dl_enrolled_{selected_course['id']}"
                )
